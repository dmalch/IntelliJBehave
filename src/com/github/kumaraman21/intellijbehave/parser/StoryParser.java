...
public class StoryParser implements PsiParser {
  ...
  /* ********************************************************** */
  // narrative WHITE_SPACE scenario* | BAD_CHARACTER
  ...
  // narrative WHITE_SPACE scenario*
  private static boolean FILE_0(PsiBuilder builder_, int level_) {
    ...
    result_ = result_ && FILE_0_2(builder_, level_ + 1);
    exit_section_(builder_, marker_, null, result_);
    return result_;
  }

  // scenario*
  private static boolean FILE_0_2(PsiBuilder builder_, int level_) {
    if (!recursion_guard_(builder_, level_, "FILE_0_2")) return false;
    int pos_ = current_position_(builder_);
    while (true) {
      if (!scenario(builder_, level_ + 1)) break;
      if (!empty_element_parsed_guard_(builder_, "FILE_0_2", pos_)) break;
